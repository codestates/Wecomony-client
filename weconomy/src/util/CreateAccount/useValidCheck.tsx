interface props {
 ...
 const validCheck = (income1:props, income2:props, outcome1:props, outcome2:props, outcomeCounter:number, incomeCounter:number, selectedMeet:any, selectedDate:any) => {
    ...
    if (selectedMeet === "선택해주세요") {
      ...
      if (outcomeCounter === 1) { // 지출 항목이 1개일 때
        if (outcome1.category === "선택해주세요" || !outcome1.cost || !outcome1.desc) {
            obj.error = "지출 칸에 작성하지 않은 부분이 있습니다."
        }
      } else if (outcomeCounter > 1) { // 지출 항목이 2개 일 때
        if (outcome1.category === "선택해주세요" || !outcome1.cost || !outcome1.desc) {
           obj.error = "지출 칸에 작성하지 않은 부분이 있습니다."
        } else if (outcome2.category === "선택해주세요" || !outcome2.cost || !outcome2.desc) {
            obj.error = "지출 칸에 작성하지 않은 부분이 있습니다."
        }
      }
     ...
     if (!outcome1.cost && !outcome1.desc) {
      if (incomeCounter === 1) {
        if (income1.category === "선택해주세요" || !income1.cost || !income1.desc) {
            obj.error = "수입 칸에 작성하지 않은 부분이 있습니다."
        }
      } else if (incomeCounter > 1) {
        if (income1.category === "선택해주세요" || !income1.cost || !income1.desc) {
            obj.error = "수입 칸에 작성하지 않은 부분이 있습니다."
        } else if (income2.category === "선택해주세요" || !income2.cost || !income2.desc) {
            obj.error = "수입 칸에 작성하지 않은 부분이 있습니다."
        }
      }
